<footer class="container-fluid d-flex justify-content-center">
    <div class="col-sm-6 col-lg-2">
        <a href="https://www.thecasinos.com">
            <img src="/img/logo.png" alt="Logo">
        </a>
    </div>
    <div class="column-description col-sm-6 col-lg-2">
        <p>With TheCasinos.com, enjoy the premier online guide reference. Explore a world of top-tier gaming and enhance your onsite casino journey. Trust in our curated selections to elevate your gaming adventure.</p>
    </div>
    <div class="column col-sm-6 col-lg-2">
        <a href="about.php">About us</a>
        <a href="terms.php">Terms and conditions</a>
        <a href="policy.php">Private policy</a>
    </div>
    <div class="column col-sm-6 col-lg-2">
        <a href="#" class="btn-download btn-ios">
            <i class="fab fa-apple" style="margin-right: 10px;"></i> iOS
        </a>
        <a href="#" class="btn-download btn-android">
            <i class="fab fa-android" style="margin-right: 10px;"></i> Android
        </a>
    </div>
</footer>

<div class="copyright-section container-fluid d-flex justify-content-around ">
    <div class="col-sm-6 col-lg-2" ><p>&copy; 2023 TheCasinos.com - Online reference to onsite experience</p></div>
    <div class="col-sm-6 col-lg-2"><a href="#" class="contact-btn">Contact</a></div>
</div>

<meta name="_appUrl" content="{{ config('app.url')}}"/>




