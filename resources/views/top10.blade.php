    ...
    @foreach ($casinosOnLineActif as $key => $casino)
        ...
        <tr class="@if($key >= $lines)  'hidden-row' @endif ">
            @foreach ($columns as $col)
                @switch ($col)
                    @case( 'Logo')
                        <td class='casino-logo'><img loading="lazy" width="50px" height="50px"
                                                     src='{{env('APP_URL').$casino->icone}}' alt='Casino Logo'></td>
                        @break
                    @case( 'Brand')
                    ...
                    @case( 'Note')
                        ...
                        @break
                    @case( 'Review')
                        <td class='casino-review'><a
                                href='{{ route('casino-online', ['name' => $casino->nom_casino_slug]) }}'>Review</a>
                        </td>
                        @break
                    @case( 'Casino')
                        <td class='casino-link'><a href='{{$casino->register_link}}'>
                                <button>Play</button>
                            </a></td>
                        @break
                @endswitch
